...
- `site/assets/social-card.webp` is a 1200 × 630 crop derived locally from that original blueprint. `site/assets/apple-touch-icon.png` is derived from the hand-authored sample document mark.
- `examples/demo/rendered/` contains original sample review files. The PDF and PNG were rendered locally in headless Chromium from hand-authored HTML/CSS; the SVG was hand-authored. All use the product palette and are MIT-licensed with the repository.
